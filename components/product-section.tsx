...
export function ProductSection({ title, products }: { title: string; products: Product[] }) {
  ...
  const handleAdd = useCallback((e: React.MouseEvent, p: Product, weightKg: 1 | 2) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleUpdateQuantity = useCallback((e: React.MouseEvent, p: Product, weightKg: 1 | 2, quantity: number) => {
    e.preventDefault();
    e.stopPropagation();
        ...
        >
          ...
          <div className="p-4 flex-1">
            ...
            <div className="mt-2 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
              <p className="text-primary font-semibold">₹{(p.price * weightKg).toFixed(2)}</p>
          ...
          <div className="p-4 pt-0 card-actions">
            {cartItem ? (
              ...
              <Button className="w-full bg-primary text-primary-foreground hover:opacity-90" onClick={(e) => handleAdd(e, p, weightKg)}>
